...
def Create_Robot():
    ...
    pyrosim.Send_Cube(name="Torso", pos=[0,0.0,2], size=[length,width,height])

    pyrosim.Send_Joint(name="Torso_FrontLeg", parent="Torso", child="FrontLeg", type="revolute", position=[0.0,0.5,1.0])
    pyrosim.Send_Cube(name="FrontLeg", pos=[0,0.5,0.0], size=[length,width,height])

    pyrosim.Send_Joint(name="Torso_BackLeg", parent="Torso", child="BackLeg", type="revolute", position=[0.0,-0.5,1.0])
    pyrosim.Send_Cube(name="BackLeg", pos=[0,-0.5,0.0], size=[length,width,height])
    pyrosim.End()
